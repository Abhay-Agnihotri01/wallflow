    ...
    AlertDialog(
        ...
        confirmButton = {
            DialogButton(stringResource(android.R.string.ok)) {
                if (changeInterval < 15) {
                    context.toast(context.getString(R.string.minimum_interval_error))
                    return@DialogButton
                }
        ...
        text = {
            ...
            Column {
                ...
                var intervalMinutes by remember {
                    mutableIntStateOf(changeInterval % 60)
                }
                LaunchedEffect(intervalDays, intervalHours, intervalMinutes) {
                    changeInterval = ((intervalDays * 24) + intervalHours) * 60 + intervalMinutes
                }
                Row(
